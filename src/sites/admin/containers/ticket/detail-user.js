...
class DetailUser extends React.Component {
    ...
    getdetailUser() {
        ...
        //         this.setState({
        //             dataProfile: s.data.medicalRecords,
        //         })
        //     }
    ...
    checkNameIncrease() {
        ...
        var sort = dataProfile.sort((itemA, itemB) => {
            const genreA = (itemA.name || "").toUpperCase();
            const genreB = (itemB.name || "").toUpperCase();
            let comparison = 0;
            if (genreA > genreB) {
                comparison = 1;
    ...
    checkEmailIncrease() {
        const { dataProfile } = this.state;
        var sort = dataProfile.sort((itemA, itemB) => {
            const genreA = (itemA.mail || "").toUpperCase();
            const genreB = (itemB.mail || "").toUpperCase();
            let comparison = 0;
            if (genreA > genreB) {
                comparison = 1;
            } else if (genreA < genreB) {
                                                        ...
                                                        dataProfile && dataProfile.length ? dataProfile.map((item, index) => {
                                                            return (
                                                                <tr
                                                                    ...
                                                                    <td>{index + 1}</td>
                                                                    <td>{item.name}</td>
                                                                    <td>{item.gender == 0 ? 'Nữ' : item.gender == 1 ? 'Nam' : item.gender == 2 ? 'Không xác định' : ''}</td>
                                                                    <td>{moment((item.dob) || {}).format("DD/MM/YYYY")}</td>
                                                                    <td>{item.mail}</td>
                                                                    {
                                                                        item.avatar ?
                                                                            <td onClick={() => this.modalDetailImage(item)} style={{ textDecoration: "underline" }}>Ảnh</td> :
                                                                            <td></td>
                                                                    }
                                                                </tr>
                                                            );
