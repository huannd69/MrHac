...
import ModalAddUpdate from './create-update-medicine-category';
...
loadPage() {
    ...
    debugger
    medicineProvider.search(params).then(s => {
...
modalCreateUpdate(item) {
  if (item) {
    if (this.state.updateable) {
      this.setState({
        modalAdd: true,
        dataMedicineCategory: item.medicineCategory,
      })
    } else {
  ...
  delete(type) {
    ...
    if (type == 1) {
      medicineProvider.delete(this.state.tempDelete.medicineCategory.id).then(s => {
        if (s && s.data && s.code == 0) {
          toast.success("Xóa " + this.state.tempDelete.medicineCategory.name + " thành công!", {
            position: toast.POSITION.TOP_RIGHT
          });
  ...
  render() {
    const { classes } = this.props;
    const { data, page, size, total, progress, stt, dataMedicineCategory, name, createable, viewable, updateable, tempDelete, deleteable } = this.state;
    return (
      <div>
        <Paper className={classes.root + ' admin-page special-page'}>
        {
          ...
          <div className={classes.tableWrapper}>
            <h2 className="title-page">Danh mục loại thuốc</h2>
                  ...
                  data && data.length ? data.map((item, index) => {
                    return (
                      <TableRow
                        ...
                        <TableCell style={{ wordBreak: "break-all"}}>{item.medicineCategory.name}</TableCell>
                        <TableCell style={{ wordBreak: "break-all"}}>{item.medicineCategory.descirption}</TableCell>
                        {/* <TableCell><img src={item.medicineCategory.linkImages}  alt="" className="image-icon"/></TableCell> */}
                        <TableCell>{moment(item.medicineCategory.updatedDate).format("DD-MM-YYYY")}</TableCell>
                        <TableCell>
                          {
                            updateable ?
        ...
        {this.state.confirmDialog && <ConfirmDialog title="Xác nhận" content={"Bạn có chắc chắn muốn xóa loại thuốc " + tempDelete.medicineCategory.name + " ra khỏi danh sách?"} btnOk="Xác nhận" btnCancel="Hủy" cbFn={this.delete.bind(this)} />}
        {this.state.modalAdd && <ModalAddUpdate data={dataMedicineCategory} callbackOff={this.closeModal.bind(this)} />}
      </div>
    );
  }
}
...
export default withStyles(styles)(connect(mapStateToProps)(Medicine));