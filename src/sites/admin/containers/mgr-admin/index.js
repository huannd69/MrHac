...
import ModalAddUpdate from './create-update-admin';
import ModalDetailtAdmin from './detail-admin';
...
class MgrAdmin extends Component {
    ...
    roleProvider.search(params).then(s => {
      if (s && s.code == 0 && s.data) {
        ...
        for (var i = 0; i < s.data.data.length; i++) {
          ...
          dataTemp2.push(s.data.data[i])
        }
        this.setState({
          listRoleAdd: dataTemp,
          listRole: dataTemp2,
        })
      }
    }).catch(e => {
  ...
  modalDetailAdmin(item) {
    if (this.state.viewable){
      this.setState({ modalDetailtAdmin: true, dataUser: item, })
    } else {
      toast.error("Unauthorized error!", {
        position: toast.POSITION.TOP_RIGHT
      });
  ...
  modalSetPassword(item) {
    this.setState({ modalSetPassword: true, dataUser: item, })
  }
  ...
  renderChirenToolbar() {
    ...
    const { listRole, roleId, active, createable } = this.state;
    return (
      <div cl>
        <TextField
          ...
          // multiline
          placeholder="Tên, Email, Số điện thoại, Username"
  ...
  render() {
    const { classes } = this.props;
    const { data, page, size, total, progress, stt, dataUser, listRole, listRoleAdd , viewable, updateable, createable} = this.state;
    return (
      <div>
        <Paper className={classes.root + ' admin-page mgr-admin-page'}>
        {
          viewable ?
          <div className={classes.tableWrapper}>
            <h2 className="title-page">Tài khoản nhân viên</h2>
                  ...
                  data && data.length ? data.map((item, index) => {
                    return (
                      <TableRow
                        ...
                        <TableCell onClick={() => this.modalDetailAdmin(item)}>{item.role && item.role.name}</TableCell>
                        <TableCell onClick={() => this.modalDetailAdmin(item)}>{item.user.username}</TableCell>
                        ...
                        <TableCell>
                          ...
                          {/* <IconButton onClick={() => this.modalSetPassword(item)} color="primary" className={classes.button} aria-label="IconRefresh">
                            <IconRefresh />
                          </IconButton> */}
                        </TableCell>
                      </TableRow>
        ...
        {this.state.modalAdd && <ModalAddUpdate data={dataUser} role={listRoleAdd} callbackOff={this.closeModal.bind(this)} />}
        {this.state.modalDetailtAdmin && <ModalDetailtAdmin data={dataUser} role={listRoleAdd} callbackOff={this.closeModal.bind(this)} />}
